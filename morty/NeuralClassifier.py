...
class NeuralClassifier:
    def __init__(self, step_size, smooth_factor, mode_names, weights=None):
        ...
        for idx, mode in enumerate(mode_names):
        	self.mode_dict[mode] = [1 if(i == idx) else 0 for i in range(len(mode_names))]
        

    def train(self, mode_labels, pitch_files, tonic_freqs, learn_rate, save_dir=''):
        tonic_freqs = [np.array(tonic) for tonic in tonic_freqs]
        pitch_tracks = ModeFun.parse_pitch_track(pitch_files, multiple=True)
        pdb.set_trace()
        cent_tracks = [Converter.hz_to_cent(k, ref_freq=tonic) \
                       for k in pitch_tracks]
    ...
    def mode_estimate(self, pitch_file, tonic_freq):
    	...
    	pcd_vals = PitchDistribution.from_cent_pitch(cent_track, 
                      tonic_freq, self.smooth_factor, 
                      self.step_size).to_pcd().vals
